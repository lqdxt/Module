...
local Queue = (function()
	...
	if not isRealFS and RunService:IsServer() then
		pcall(function()
			StudioStore = DataStoreService:GetDataStore("PlayerDataStore_v3")
		end)
	end
	...
	local function FlushFilePath(filePath)
		...
		if content ~= nil then
			if SaveToStudioStore(filePath, content) then
		...
		task.spawn(function()
			while autoSaveRunning do
				...
				if next(dirtyFiles) then
					for filePath in pairs(dirtyFiles) do
						if not HasDataStoreBudget(Enum.DataStoreRequestType.UpdateAsync) then
							warn("Auto-save paused: DataStore request budget low.")
							break
						end
